...
- **Deletion:**
  - Managed SGs are deleted when the corresponding NLB is deleted. The controller uses exponential backoff to handle AWS dependency violations.
...
```
2. Create the BYO SG (Test 2, step 1). Controller logs:
```sh
$ echo $SG_ID
